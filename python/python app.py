...
def birthday():
    ...
    # Birthday
        ...
        if month == 2 and day > 28:
            flash(f"February does not have {day} days")
            return redirect(url_for("birthday"))
        today = datetime.now().timetuple().tm_yday
        birthday = datetime(datetime.now().year, month, day).timetuple().tm_yday
        if month not in big_month and day >30:
            flash(f"Month {month_map[month]} has at most 30 days.", "error")
            return redirect(url_for("birthday"))
        if birthday > today:
            message = f"Dear {name}, your birthday is on {month_map[month]} {str(day)} it will be your birthday in {str(birthday-today)} days"
